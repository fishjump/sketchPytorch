...
def resize_img_512_3d(img):
    zeros = np.zeros((3,1,512,512), dtype=np.float)
    zeros[0 : img.shape[0] , 0 , 0 : img.shape[1] , 0 : img.shape[2]] = img
    return zeros

def show_active_img_and_save(name,img,path):
    mat = get_colored_sketch(img)
    cv2.imshow(name,mat)
